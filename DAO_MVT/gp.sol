...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    contract GreenPointLandReserves{
        ...
        bool firstBump = true;
        event Sell_MVT(uint mvtSold, uint cashout);
        ...
        mapping(uint => bool) totemManifest;
        ...
        function unstakeNFT(uint tokenID) public{
            address sender = msg.sender;
            require(staker[tokenID] == sender && !requestLocked[tokenID]);
        ...
        function viewTotems(address account, uint[] memory totems) public view returns(uint[] memory tokenIDs, bool[] memory accountIsCurrentlyStaking, uint[] memory acreWeight, bool[] memory owned, bool[] memory manifested, bool[] memory staked, uint[] memory manifestEarnings,bool[] memory pendingManifest,address[] memory holder ){
            uint L;
            if(totems.length==0){
